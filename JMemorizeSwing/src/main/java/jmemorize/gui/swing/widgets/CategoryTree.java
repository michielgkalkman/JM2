...
import jmemorize.gui.swing.actions.file.SaveCategoriesAsAction;
...
public class CategoryTree extends JTree implements CategoryObserver,
	...
	private class CellEditor extends DefaultTreeCellEditor {
		...
		public boolean isCellEditable(EventObject event) {
			...
			if (event != null) {
				...
				if (path != null) {
					m_editedNode = (DefaultMutableTreeNode) path
							.getLastPathComponent();
			...
			return super.isCellEditable(event)
					&& m_editedCategory != m_rootCategory;
		}
	...
	private final ImageIcon FOLDER_ICON = new ImageIcon(getClass().getResource(
			"/resource/icons/folder.gif")); //$NON-NLS-1$
	...
	public CategoryTree( JMemorizeIO jMemorizeIO) {
	...
	public void setSelectedCategory(Category category) {
		...
		while (enumer.hasMoreElements()) {
			DefaultMutableTreeNode node = enumer
					.nextElement();
	 ...
	 *         the case if the category was selected by right-clicking on the
	 *         node.
	 */
	...
	public void onCategoryEvent(int type, Category category) {
		...
		case ADDED_EVENT:
			...
			int idx = category.getParent().getChildCategories()
					.indexOf(category);
	...
	public List<Category> getSelectedCategories() {
		...
		if (treePaths != null) {
			for (final TreePath treePath : treePaths) {
				final DefaultMutableTreeNode node = (DefaultMutableTreeNode) treePath
						.getLastPathComponent();
				categories.add((Category) node.getUserObject());
			}
		}
			...
			public void mouseClicked(MouseEvent e) {
				if (SwingUtilities.isRightMouseButton(e)) {
					...
					if (row > -1) {
						...
						m_categoryMenu.show(e.getComponent(), e.getX(),
								e.getY());
					}
				}
			}
	...
	private JPopupMenu buildCategoryContextMenu() {
		...
		menu.add(new ActionWrapper(new SaveCategoriesAsAction(this, jMemorizeIO)));
	...
	private DefaultMutableTreeNode getNode(Object userValue) {
		...
		for (Enumeration<DefaultMutableTreeNode> enumer = root.depthFirstEnumeration(); enumer
				.hasMoreElements();) {
			DefaultMutableTreeNode node = enumer
					.nextElement();
