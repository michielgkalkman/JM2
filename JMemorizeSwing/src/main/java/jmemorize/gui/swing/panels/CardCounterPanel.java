...
public class CardCounterPanel extends JPanel implements LearnSessionObserver,
	 ...
	 * 
	 */
	private static final long serialVersionUID = -5435813119332561649L;
	...
	public void sessionStarted(final LearnSession session) {
		...
		final LearnSettings settings = session.getSettings();
	 ...
	 */
	@Override
	public void nextCardFetched(final Card card, final boolean flippedMode) {
		final int cardsLearned = m_session.getNCardsLearned();
		final int cardsPartiallyLearned = m_session.getNCardsPartiallyLearned();

		setCardsPassed(cardsLearned, cardsPartiallyLearned);
	}

	private void start(final int target) {
	...
	private void setCardsPassed(final int passed, final int partiallyPassed) {
		...
		if (m_cardsTarget > -1) {
			if (USE_PIECHART) {
				final int cardsUnlearned = m_cardsTarget
						- (m_cardsPassed + m_cardsPartiallyPassed);
				m_pieDataset.setValue(Localization.get(LC.STATUS_LEARNED),
						m_cardsPassed);
				m_pieDataset.setValue(Localization.get(LC.STATUS_PARTIAL),
						m_cardsPartiallyPassed);
				m_pieDataset.setValue(Localization.get(LC.STATUS_UNLEARNED),
						cardsUnlearned);
			} else {
				if (!USE_PARTIAL_BAR)
				...
				if (USE_EXTENT_BAR) {
					final ExtentProgressBar bar = (ExtentProgressBar) m_bar;
	...
	private void initComponents(final int target) {
		removeAll();
	...
	private ChartPanel buildPiePanel() {
		final JFreeChart chart = ChartFactory.createPieChart(null,
				m_pieDataset, true, false, false);
	...
	private void setupPieLegend(final JFreeChart chart) {
		final LegendTitle legend = chart.getLegend();
		final Font legendFont = legend.getItemFont();
		legend.setItemFont(legendFont.deriveFont(Font.BOLD, 20));
	}

	private void setupPiePlot(final PiePlot plot) {
		...
		plot.setLabelGenerator(new PieSectionLabelGenerator() {
			...
			public String generateSectionLabel(final PieDataset arg0,
					final Comparable arg1) {
				return null;
			}
		...
		plot.setSectionPaint(Localization.get(LC.STATUS_LEARNED),
				ColorConstants.LEARNED_CARDS);
		plot.setSectionPaint(Localization.get(LC.STATUS_PARTIAL),
				ColorConstants.PARTIAL_LEARNED_CARDS);
		plot.setSectionPaint(Localization.get(LC.STATUS_UNLEARNED),
				ColorConstants.UNLEARNED_CARDS);
	}
	...
	private ExtentProgressBar buildExtentProgressBar() {
		...
		final Color grn = Color.GREEN.darker();
		final Color transparentGreen = new Color(grn.getRed(), grn.getGreen(),
				grn.getBlue(), 128);
	...
	/**
	 * @return The string that is used to show the number of cards in
	 *         progressbar.
	 */
