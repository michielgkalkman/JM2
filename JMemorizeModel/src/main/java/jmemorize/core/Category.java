 ...
 * <b>expired</b>, depending on the fact of wether their expiration timer has
 * already passed by or not.
 * 
 ...
 * a card CB. Getting all cards of category A will then return CA <i>and</i> CB.
 * 
...
public class Category implements Events {
	...
	public boolean equals(final Object object) {
		...
		if (object == null) {
			...
			if (getName().equals(category.getName())) {
				if (getParent() == null) {
					fEquals = category.getParent() == null;
				} else if (category.getParent() == null) {
					fEquals = false;
				} else {
					fEquals = getParent().equals(category.getParent());
				}
			} else {
				fEquals = false;
			}
	...
	private List<List<Card>> m_decks = new ArrayList<List<Card>>(); // list of
																	// card
																	// lists
	...
	public static void resetCardLevel(Card card, Date testDate) {
		card.incStats(0, 1);
		changeCardLevel(card, 0, testDate, null); // CHECK use null for
													// testdate!?
	}
	 ...
	 *         categories. Returns all cards of all decks if -1 is given as
	 *         level.
	 */
	public List<Card> getCards(int level) {
		...
		cardList.addAll(getChildCategories().stream().filter(child -> (child.getNumberOfDecks() > level))
				.flatMap(c -> c.getCards(level).stream()).collect(Collectors.toList()));
	 ...
	 *         and its child categories. Returns the total card count of all
	 *         decks if -1 is given as level.
	 */
	...
	public List<Card> getExpiredCards() {
		return getCards().stream().filter(card -> card.isExpired()).collect(Collectors.toList());
	}
	 ...
	 * 
	 * @param level
	 *            the level of the deck of who's cards you want to get.
	...
	/**
	 * @return All cards that are local to this category. That is all cards that
	 *         directly belong to this category and not to any of this child
	 *         categories.
	 */
	 ...
	 *         all cards that directly belong to this category and not to any of
	 *         this child categories.
	 */
	 ...
	 *         That means that no child categoriy can have more number of decks
	 *         then its parent category.
	 */
	...
	/**
	 * @return the child category with given name. <code>null</code> if there is
	 *         child category with given name.
	 */
	...
	/**
	 * @return True if given category is a child of this category. False
	 *         otherwise.
	 */
	 ...
	 *         Starting from the root, every category in the path is separated
	 *         with a slash. The path includes the name of this category as last
	 *         part.
	 */
	...
	public int getDepth(final int depth) {
		return m_parent == null ? depth : m_parent.getDepth(depth + 1);
	}

	/**
	 * @return A list of all child categories and their childs etc.
	 */
	public List<Category> getSubtreeList() // TODO rename to
											// getChildCategoriesTree
	{
	 ...
	 *         categories and the same cards as this category, but without any
	 *         user dependent-stats(e.g. all cards are at level 0 and have no
	 *         date_tested).
	 */
	 ...
	 */
	private static void changeCardLevel(Card card, int newLevel, Date newTest, Date newExpiration) {
		Category category = card.getCategory();
		int level = card.getLevel();
