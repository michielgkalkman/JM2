 ...
 * <li>The learn session waits for a call to either {
 * {@link #cardChecked(boolean, boolean)} or {@link #cardSkipped()}. This makes
 * the learn session perform some action on the card.</li>
 ...
 * 
 * The order of a card in the learn session depends on the shuffle and category
 * order settings: [Shuffle: Off, Category Order: Off] Deck, Last test date
 * [Shuffle: Off, Category Order: On ] Category, Deck, Last test date [Shuffle:
 * On, Category Order: Off] Deck, Random number [Shuffle: On, Category Order: On
 * ] Category, Deck, Random number
 * 
 * @author djemili
 */
public class DefaultLearnSession implements CategoryObserver, LearnSession {
	/**
	 * A Comparator that is used for sorting cards in learn sessions. This is
	 * used to sort the cards into equivalence classes, from which the next card
	 * will be drawn randomly.
	 */
	private class CardComparator implements Comparator<CardInfo> {
		private final Map<Category, Integer> m_categoryGroupOrder;

		public CardComparator(final Map<Category, Integer> categoryGroupOrder) {
		 ...
		 */
		@Override
		public int compare(final CardInfo card0, final CardInfo card1) {
			...
			if (m_settings.isGroupByCategory()) {
				final Integer cat0 = m_categoryGroupOrder.get(card0
						.getCategory());
				final Integer cat1 = m_categoryGroupOrder.get(card1
						.getCategory());
	...
	private final LearnSettings m_settings;
	private final LearnSessionProvider m_provider;
	...
	private final List<Card> m_cardsChecked = new ArrayList<Card>();
	private final Set<Card> m_cardsLearned = new HashSet<Card>();
	private final Map<Card, CardInfo> m_cardsInfoMap = new HashMap<Card, CardInfo>();
	...
	private final Set<Card> m_cardsEverFailed = new HashSet<Card>();
	private final Set<Card> m_cardsSkipped = new HashSet<Card>();

	// NOTE - this is only the *active* cards which are partially learned -
	// there may be others in the reserve set.
	private final Set<Card> m_cardsActivePartiallyLearned = new HashSet<Card>();
	...
	private final List<LearnCardObserver> m_cardObservers = new LinkedList<LearnCardObserver>();

	private Date m_start;
	private Date m_end;

	private final Logger m_logger = Logger.getLogger("jmemorize.session");

	/**
	 * Creates a new learn session. Use {@link #startLearning()} to start the
	 * learning.
	 */
	public DefaultLearnSession(final Category category,
			final LearnSettings settings, final List<Card> selectedCards,
		...
		final Map<Category, Integer> order = m_settings.isGroupByCategory() ? createCategoryGroupOrder()
				: null;

		m_cardsActive = fetchCards(selectedCards, learnUnlearned, learnExpired,
				order);
		m_cardsReserve = new EquivalenceClassSet<CardInfo>(
				m_cardsActive.getComparator());
		// Note that EquivalenceClassSets always default to shuffle mode (any
		// card
		// from the current class may be chosen next.) This is what we want
		// here.
	}
	...
	public void startLearning() {
		if (m_learningStarted)
			throw new IllegalStateException(
					"startLearning should only happen once!");

		m_learningStarted = true;
		m_start = new Date();

		// move all cards to cardsPastLimit, then fetch exactly as many as
		// needed
	 ...
	 */
	@Override
	public void cardChecked(final boolean passed, final boolean shownFlipped) {
		final Card currentCard = m_currentCardInfo.getCard();

		m_logger.fine(String.format("cardChecked: %b %s", passed, currentCard
		...
		if (passed) {
			...
			if (m_settings.getSidesMode() == LearnSettings.SIDES_BOTH) {
				...
				if ((frontAmountLearned < m_settings.getAmountToTest(true))
						|| (backAmountLearned < m_settings
								.getAmountToTest(false))) {
			 ...
			 * equivalence class (i.e. the set is in an inconsistent state). We
			 * can't fix it until after the reset, *but* the resetCardLevel()
			 * method fires the event which results in the observers reacting
			 * (checking for end of session, getting the next card, etc). The
			 * card's equivalence class will be wrong, but this should not be a
			 * problem for gotoNextCard. We reset the equivalence class as soon
			 * as possible.
			 */
		...
		m_logger.fine("...Cards partially learned: "
				+ getNCardsPartiallyLearned());
	...
	public void cardSkipped() {
		...
		if (m_cardsReserve != null && m_cardsReserve.size() > 0) {
			...
			final CardInfo replacementCardInfo = m_cardsReserve.loopIterator()
					.next();
			final Card replacementCard = replacementCardInfo.getCard();
	...
	public Set<Card> getPassedCards() {
		...
		final Set<Card> tempSet = new HashSet<Card>(m_cardsLearned);
	...
	public Set<Card> getFailedCards() {
		final Set<Card> tempSet = new HashSet<Card>(m_cardsEverFailed);
	...
	public void onCardEvent(final int type, final Card card,
		...
		switch (type) {
		case ADDED_EVENT:
			// if there is a reserve and we have enough cards, add to the
			// reserve
			final int allCards = m_cardsLearned.size() + m_cardsActive.size();
			if (m_settings.isCardLimitEnabled()
					&& allCards >= m_settings.getCardLimit()) {
		...
		case DECK_EVENT:
			...
			// TODO currently, resetting a learned card does not put it back in
			// the
	 ...
	 */
	@Override
	public void addObserver(final LearnCardObserver observer) {
	 ...
	 */
	@Override
	public void removeObserver(final LearnCardObserver observer) {
	...
	public boolean isQuit() {
		final boolean noCardsLeft = m_cardsActive.size() == 0;
		final boolean limitReached = m_settings.isCardLimitEnabled()
				&& m_cardsLearned.size() >= m_settings.getCardLimit();

		return m_quit || noCardsLeft || limitReached;
	}

	private void raiseCardLevel(final Card card) {
		final CardInfo cardInfo = getCardInfo(card);
		...
		final Date expiration = m_settings.getExpirationDate(m_start, level);
	...
	private void gotoNextCard() {
		...
		if (isQuit()) {
			...
			final boolean flippedMode = checkIfFlipped();
			for (final LearnCardObserver observer : m_cardObservers) {
	...
	private boolean checkIfFlipped() {
		if (m_settings.getSidesMode() == LearnSettings.SIDES_RANDOM) {
			...
			int timesToLearnFront = m_settings.getAmountToTest(true)
					- currentCard.getLearnedAmount(true);

			int timesToLearnBack = m_settings.getAmountToTest(false)
			...
			final int rand = m_rand.nextInt(timesToLearnFront
					+ timesToLearnBack);
	 ...
	 * params.
	 */
	private EquivalenceClassSet<CardInfo> fetchCards(
			final List<Card> selectedCards, final boolean learnUnlearnedCards,
			final boolean learnExpiredCards,
			final Map<Category, Integer> categoryGroupOrder) {
		...
		for (final Card card : cards) {
			final CardInfo cardInfo = new CardInfo(card);
		...
		if (levels.size() > 1) {
			for (int i = 0; i < shuffledCardsCount; i++) {
				...
				final CardInfo cardInfo = cardInfos.remove(randIndex);
		...
		final EquivalenceClassSet<CardInfo> cardSet = new EquivalenceClassSet<CardInfo>(
				new CardComparator(categoryGroupOrder));
	...
	private Set<Card> toCardSet(final Collection<CardInfo> cardInfos) {
		final HashSet<Card> set = new HashSet<Card>();
		for (final CardInfo cardInfo : cardInfos) {
	...
	private CardInfo getCardInfo(final Card card) {
		return m_cardsInfoMap.get(card);
	}

	/**
	 * @return Cards that are being learned can be grouped by categories. In
	 *         this case the map holds for every category the position when it
	 *         should appear.
	 */
	private Map<Category, Integer> createCategoryGroupOrder() {
		final List<Category> categories = m_category.getSubtreeList();

		if (m_settings.getCategoryOrder() == LearnSettings.CATEGORY_ORDER_RANDOM) {
			Collections.shuffle(categories);
		}

		final HashMap<Category, Integer> map = new HashMap<Category, Integer>();
		int i = 0;
